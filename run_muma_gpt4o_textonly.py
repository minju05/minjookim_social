...
from muma_gpt4o_textonly import predict_question, prediction_to_dict
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run MuMA-ToM text-only baseline (우리 버전 prompt, no video frames).")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/muma_gpt4o_textonly_ours_predictions.jsonl"),
    )
    return parser
...
def main() -> None:
    ...
    with args.output.open("a", encoding="utf-8") as f:
        for record in tqdm(selected, desc="text-only (우리 버전)"):
