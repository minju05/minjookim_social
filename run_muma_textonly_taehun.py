...
from muma_gpt4o_textonly_taehun import predict_question, prediction_to_dict
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run MuMA-ToM text-only with 태훈 버전 prompt.")
    parser.add_argument("--limit", type=int, default=100)
    parser.add_argument("--offset", type=int, default=0)
    parser.add_argument("--per-type", type=int, default=40, help="N questions per question type.")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/muma_textonly_taehun_per40.jsonl"),
    )
    return parser
...
def main() -> None:
    ...
    with args.output.open("a", encoding="utf-8") as f:
        for record in tqdm(selected, desc="text-only (태훈 버전)"):
            if record.question_id in done_ids:
                continue
            prediction = predict_question(client, settings, record)
