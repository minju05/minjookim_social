...
from muma_gpt4o_textonly_minjoo import predict_question, prediction_to_dict
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run MuMA-ToM text-only with 민주 버전 prompt.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/muma_textonly_minjoo_per40.jsonl"),
    )
    return parser
...
def main() -> None:
    ...
    with args.output.open("a", encoding="utf-8") as f:
        for record in tqdm(selected, desc="text-only (민주 버전)"):
