...
from muma_limp_c import answer_question_c, prediction_to_dict


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run MuMA-ToM LIMP+C (Module C: Intent-Sensitive Consistency Scoring).")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/muma_limp_c_predictions.jsonl"),
    )
    parser.add_argument(
        "--actions-cache",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Consistency term weight λ. Higher = stronger Module C influence.",
    )
    return parser
...
def main() -> None:
    ...
    # Resume: load already-completed question IDs from existing output
    ...
    if args.output.exists():
        with args.output.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                predictions.append(type("P", (), row)())  # lightweight placeholder for summarize
        print(f"Resuming: {len(done_ids)} questions already done, skipping.")

    with args.output.open("a", encoding="utf-8") as f:
        for record in tqdm(selected, desc="MuMA-ToM LIMP+C"):
            ...
            if not raw_action_text:
                try:
                    ...
                    raw_action_text = extract_action_text(gemini_client, settings, uploaded, prompt, fps=args.fps)
            ...
            prediction = answer_question_c(
                openai_client, record, raw_action_text, lambda_weight=args.lambda_weight
            )
            predictions.append(prediction)
            row = prediction_to_dict(prediction)
            f.write(json.dumps(row, ensure_ascii=False) + "\n")
            f.flush()

    # Re-load all predictions from file for accurate summary
    final_predictions = []
    with args.output.open("r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
    ...
    # Build summary from jsonl directly
    ...
    summary = {
        ...
        "overall_accuracy": sum(r["correct"] for r in final_predictions) / len(final_predictions) if final_predictions else 0.0,
        "accuracy_by_type": {qt: by_type_correct[qt] / total for qt, total in sorted(by_type_total.items())},
